...
type AuthAction =
  | { type: 'AUTH_START' }
  | { type: 'AUTH_SUCCESS'; payload: { user: User; tenant: Tenant; token: string | null } }
...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    const initializeAuth = async () => {
      ...
      dispatch({
        type: 'AUTH_SUCCESS',
        payload: { user, tenant, token: authService.getToken() },
      });
      } catch (error: any) {
  ...
  const handleSuccess = (response: AuthResponse) => {
    ...
    dispatch({
      type: 'AUTH_SUCCESS',
      payload: { user: response.user, tenant: response.tenant, token: response.token },
    });
  };
